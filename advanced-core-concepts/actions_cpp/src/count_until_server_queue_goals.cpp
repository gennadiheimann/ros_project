...
public:
  ...
  {
    goal_queue_thread_ = std::thread(&CountUntilServer::run_goal_gueue_thread, this);
...
private:
  ...
  void run_goal_gueue_thread(){
    ...
    while(rclcpp::ok()){
      ...
      if(next_goal){
        RCLCPP_INFO(this->get_logger(), "Excute next goal in queue");
